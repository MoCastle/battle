...
public class BaseAction : BaseState
{
    ...
    protected virtual Vector2 MoveDir
    {
        get
        {
            return Vector2.right * (m_ActorObj.FaceDir.x > 0 ? 1 : -1);
        }
    }
    #endregion
    #region 流程
    ...
    public virtual void EffectOnActor(BaseActorObj targetActor, Collider2D TargetCollider)
    {
        ...
        targetActor.HurLife(this.m_SkillInfo.Damage);
    }
    //生成特效
    ...
    //输入向量
    public virtual void InputDirect(Vector2 dirction)
    {
        m_InputDIr = dirction;
        ChangeDir();
    }
    ...
    {
        if (!m_DirLock && m_InputDIr.x * m_ActorObj.FaceDir.x < 0)
        {
