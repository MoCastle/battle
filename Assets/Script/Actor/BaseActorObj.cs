...
namespace GameScene
{
    ...
    public abstract class BaseActorObj : MonoBehaviour
    {
        ...
        public FaceDirection HDirection
        {
            get
            {
                return m_CharacterAnim.Animator.transform.localScale.x > 0 ? FaceDirection.Right : FaceDirection.Left;
            }
            set
            {
                if (value == this.HDirection)
                    return;
                Vector3 scale = m_CharacterAnim.Animator.transform.localScale;
                switch (value)
                {
        ...
        public BoxCollider2D BodyCollider
        {
            get
            {
                Transform animatorTransform = m_CharacterAnim.transform;
                
                return animatorTransform.Find("Body").GetComponent<BoxCollider2D>();
            }
        }
        ...
        public Vector2 FaceDir
        {
            get
            {
                Vector2 dir = Vector2.right * (m_CharacterAnim.Animator.transform.localScale.x > 0 ? 1 : -1);
                return dir;
            }
        }
        public Vector3 FaceDir3D
        {
            get
            {
                Vector3 dir = Vector3.right * (m_CharacterAnim.Animator.transform.localScale.x > 0 ? 1 : -1);
                return dir;
            }
        }
        ...
        public void FaceToDir(Vector2 dir)
        {
            ...
            if (dir.x * m_CharacterAnim.Animator.transform.localScale.x < 0)
            {
                scale.x *= -1;
                this.HDirection = dir.x > 0 ? FaceDirection.Right : FaceDirection.Left;
                m_CharacterAnim.Animator.transform.localScale = scale;
            }
        ...
        #endregion
        #region 对话
        public void SayDialog(int type)
        {
            ActorDialogInfo dialog = new ActorDialogInfo();
            bool getDialog = false;
            foreach(ActorDialogInfo dialogInfo in DialogInfos)
            {
            ...
            if(getDialog)
            {
                m_DialogUI.dialogBubble.Say(dialog);
            }
        }
        #endregion
        #region 技能
        public void HurLife( float value )
        {
            m_ActorPropty.ModLifeValue(-value);
            if(m_ActorPropty.IsDeath)
            {
                m_ActionCtrler.SetTriiger("Death");
                OnDeath();
